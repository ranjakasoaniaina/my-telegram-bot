# menu/reminders/show_reminder_select_groups.py
...
async def show_reminder_select_groups(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Affiche la liste des groupes éligibles avec sélection multiple (toggle) pour un rappel."""
    ...
    if groupes:
        texte = t("reminder_group_select", lang)
    else:
    ...
    for groupe in groupes:
        ...
        boutons.append([
            InlineKeyboardButton(
                f"{prefixe} {titre}",
                callback_data=f"reminder:toggle_group:{gid}"
            )
        ])
    ...
    if selected_ids:
        ligne_actions.append(
            InlineKeyboardButton(t("validate_selection", lang), callback_data="reminder:groups_validated")
        )
    ligne_actions.append(
    ...
    context.user_data["current_screen"] = "reminder_select_groups"
