...
async def show_reminder_enter_datetime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # --- Affichage de l'erreur de validation si présente ---
    if error_msg:
        texte = f"❌ {error_msg}\n\n{texte}"
    ...
    # step_parent est déjà défini par l'appelant (select_reminder ou set_reminder_mode_*)
    # on le laisse tel quel, ou on le détermine comme avant si pas de draft
    if not current:
    ...
    # en mode édition, le step_parent doit pointer vers la liste d'édition
