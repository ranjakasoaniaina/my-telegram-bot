...
async def display_message(
    ...
    """Affiche un message en privilégiant l'édition, et envoie un nouveau message
    si nécessaire, tout en garantissant que le message du bot reste le dernier.
    """
    # Si on a un callback, on édite le message d'origine
    if update.callback_query:
        query = update.callback_query
        await query.edit_message_text(text=texte, reply_markup=clavier)
        return

    # Pas de callback → on doit afficher sans callback
    chat_id = update.effective_chat.id
    old_message_id = context.user_data.get("active_message_id")

    # Essayer d'éditer l'ancien message s'il existe encore
    if old_message_id:
        try:
            ...
            # L'édition a échoué (message supprimé, etc.), on supprime l'ancien
            try:
                await context.bot.delete_message(chat_id=chat_id, message_id=old_message_id)
            except Exception:
                pass

    # Envoyer un nouveau message (il apparaîtra en bas)
    if update.message:
